...
public class SM3_Attack extends DifferentialAnalysis {
    ...
    public void initmoudle(sm3_type t){
    	round=t.getSm3round();
    	step=t.getSm3step();
    	  sm3x1 =t.getSm3x1() ;
    	  sm3y1 = t.getSm3y1();
	...
	//	System.out.println("slect_attack_ is running");
		...
		int rounds=t.getSm3round();
    	int steps=t.getSm3step();
		...
		// 摘要计算函数
		processBlock(t,selected,rounds,steps);
		return selected;

	}
	
	public void processBlock(sm3_type tt,float[] selected,int rounds,int steps) {
...
//	 String temstr=byteArrayToHexString(temptest);
		...
		if (round == 0) { // first round
			...
			System.out.println(sm3x2);
			System.out.println(sm3x2.length());
	...
	public void report(int key, Vector<float[]> best, int fragmentOffset,
			int fragmentEnd,int rounds,int steps,String[] returnnews) {
		super.report(key, best, fragmentOffset, fragmentEnd,rounds,steps,returnnews);
		round=rounds;
		step=steps;
		System.out.println((int)best.get(0)[1]);	
